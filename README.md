...
- صفحهٔ **هشدار معامله**: فقط ستاپ واقعی + لبهٔ خارج‌از‌نمونه + کیفیت قوی/قانع‌کننده؛ بدون پیشنهاد ساختگی؛ بروزرسانی خودکار و هشدار صوتی بعد از فعال‌سازی
...
## معامله خودکار MetaTrader (دمو)

صفحهٔ **معامله خودکار** یوزر/رمز/سرور دمو را می‌گیرد و روی **همان کامپیوتری که ترمینال MT5 آلپاری باز است** معامله می‌زند.

1. MT5 آلپاری را باز و به دمو لاگین کن
2. Trendix را همانجا بالا بیاور (`python run.py`)
3. `pip install MetaTrader5` (ویندوز)
4. در UI تنظیمات را ذخیره کن → تست اتصال
5. برای حلقهٔ مداوم: `python -m app.trading.worker`

رمز فقط در `~/.trendix/mt5_local.json` ذخیره می‌شود و داخل گیت نمی‌رود. روی Render ابری معمولاً MT5 در دسترس نیست.
